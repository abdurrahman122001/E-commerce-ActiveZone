...
<div class="aiz-titlebar text-left mt-2 mb-3">
	<div class="row align-items-center">
		<div class="col-md-6">
			<h1 class="h3">{{translate('All Franchise Packages')}}</h1>
		</div>
        <div class="col-md-6 text-md-right">
            <a href="{{ route('franchise_packages.create') }}" class="btn btn-circle btn-info">
...
<div class="row">
    @foreach ($franchise_packages as $key => $package)
        <div class="col-lg-3 col-md-4 col-sm-12">
            <div class="card">
                <div class="card-body text-center">
                    ...
                    @if($package->category)
                        <p class="fs-14 text-muted">{{ translate('Category') }}: {{$package->category->getTranslation('name')}}</p>
                    @endif
                    <p class="fs-15">{{translate('Product Limit') }}:
                        <span class="text-bold">{{$package->product_limit}}</span>
                    </p>
                    <p class="fs-15">{{translate('Duration') }}:
                        <span class="text-bold">{{$package->duration}} {{ translate('Days') }}</span>
                    </p>
                    <div class="mar-top">
