...
<div class="row">
    <div class="col-lg-8 mx-auto">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('franchise.employees.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group row">
                        <label class="col-sm-3 col-from-label">{{translate('Password')}} <span class="text-danger">*</span></label>
                        <div class="col-sm-9">
                    ...
                    <div class="form-group row">
                        <label class="col-sm-3 col-from-label">{{translate('Confirm Password')}} <span class="text-danger">*</span></label>
                        <div class="col-sm-9">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control @error('role') is-invalid @enderror" name="role" value="{{ old('role') }}" placeholder="e.g. Manager, Sales Executive" required>
                            @error('role')
                                <span class="invalid-feedback" role="alert">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <select class="form-control aiz-selectpicker @error('franchise_level') is-invalid @enderror" name="franchise_level" id="franchise_level" required>
                                <option value="CITY" {{ old('franchise_level') == 'CITY' ? 'selected' : '' }}>{{translate('City Level')}}</option>
                                <option value="SUB" {{ old('franchise_level') == 'SUB' ? 'selected' : '' }}>{{translate('Sub Franchise Level')}}</option>
                            </select>
                    ...
                    <div class="form-group row" id="sub_franchise_container" style="display: none;">
                        ...
                        <div class="col-sm-9">
                            <select class="form-control aiz-selectpicker @error('sub_franchise_id') is-invalid @enderror" name="sub_franchise_id">
                                ...
                                @foreach($subFranchises as $subFranchise)
                                    <option value="{{ $subFranchise->id }}" {{ old('sub_franchise_id') == $subFranchise->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="number" step="0.01" min="0" max="100" class="form-control @error('commission_percentage') is-invalid @enderror" name="commission_percentage" value="{{ old('commission_percentage', 0) }}">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#franchise_level').trigger('change');
    });
</script>
@endsection
