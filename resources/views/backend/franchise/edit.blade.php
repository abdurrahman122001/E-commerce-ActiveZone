        ...
        <div class="card-body p-0">
            <form class="p-4" action="{{ route('admin.franchises.update', $franchise->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <input type="text" placeholder="{{translate('Pin Code')}}" id="pincode" name="pincode" value="{{ $franchise->pincode }}" class="form-control" required>
                    </div>
                </div>
                <div class="form-group row">
                    <label class="col-sm-3 col-from-label" for="address">{{translate('Address')}}</label>
                    <div class="col-sm-9">
                        <textarea name="address" rows="2" class="form-control" placeholder="{{translate('Address')}}" required>{{ $franchise->address }}</textarea>
                    </div>
                </div>
                <div class="form-group row">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        ...
                        @if($franchise->id_proof)
                            <div class="mt-2">
                                <a href="{{ asset('storage/'.$franchise->id_proof) }}" target="_blank" class="text-info">{{ translate('View Current Front') }}</a>
                            </div>
                        @endif
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        ...
                        @if($franchise->id_proof_back)
                            <div class="mt-2">
                                <a href="{{ asset('storage/'.$franchise->id_proof_back) }}" target="_blank" class="text-info">{{ translate('View Current Back') }}</a>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-5">
                        <input type="number" step="0.01" min="0" placeholder="{{translate('Commission Value')}}" id="commission_percentage" name="commission_percentage" value="{{ $franchise->commission_percentage }}" class="form-control" required>
                    </div>
                    <div class="col-sm-4">
                        <select class="form-control aiz-selectpicker" name="commission_type" id="commission_type">
                            <option value="percentage" @if($franchise->commission_type == 'percentage') selected @endif>{{translate('Percentage (%)')}}</option>
                            <option value="flat" @if($franchise->commission_type == 'flat') selected @endif>{{translate('Flat Amount')}}</option>
                        </select>
                    </div>
                </div>
