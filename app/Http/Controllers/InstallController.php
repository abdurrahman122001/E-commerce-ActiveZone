...
class InstallController extends Controller
{
    ...
    public function system_settings(Request $request) {
        \Log::info('Installation: Started system_settings');
        
        $system_default_currency = BusinessSetting::where('type', 'system_default_currency')->first();
        if ($system_default_currency) {
            $system_default_currency->value = $request->system_default_currency;
            $system_default_currency->save();
        }

        $home_default_currency = BusinessSetting::where('type', 'home_default_currency')->first();
        if ($home_default_currency) {
            $home_default_currency->value = $request->system_default_currency;
            $home_default_currency->save();
        }
        ...
        \Log::info('Installation: Settings saved, checking APP_KEY');
        
        // Only generate key if it's missing or empty
        if (empty(env('APP_KEY'))) {
            \Log::info('Installation: Generating key');
            Artisan::call('key:generate');
        }

        \Log::info('Installation: Handling Admin User');
        $user = User::where('email', $request->admin_email)->first() ?: new User;
        ...
        \Log::info('Installation: Assigning Role');
        try {
            $user->assignRole(['Super Admin']);
        } catch (\Exception $e) {
            \Log::warning('Installation: Could not assign role: ' . $e->getMessage());
        }

        \Log::info('Installation: Updating RouteServiceProvider');
        ...
        if (file_exists($newRouteServiceProvider)) {
            copy($newRouteServiceProvider, $previousRouteServiceProvider);
        }

        if (Session::has('purchase_code')) {
            $business_settings = BusinessSetting::where('type', 'purchase_code')->first() ?: new BusinessSetting;
        ...
        \Log::info('Installation: Clearing optimization');
        try {
            Artisan::call('optimize:clear');
        } catch (\Exception $e) {
            \Log::warning('Installation: Optimize clear failed: ' . $e->getMessage());
        }
        
        \Log::info('Installation: Rendering Step 6');
